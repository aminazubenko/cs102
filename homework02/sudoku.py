...
def find_possible_values(grid: List[List[str]], pos: Tuple[int, int]) -> Set[str]:
    ...
    """
    sets = set("123456789")
    blocks = set(get_block(grid, pos))
    colums = set(get_col(grid, pos))
    rows = set(get_row(grid, pos))
    return sets - rows - blocks - colums
...
def solve(grid: List[List[str]]) -> Optional[List[List[str]]]:
    ...
    """ Как решать Судоку?
        ...
        3. Для каждого возможного значения:
            ...
            3.2. Продолжить решать оставшуюся часть
    >>> grid = read_sudoku('puzzle1.txt')
    >>> solve(grid)
    [ ['5', '3', '4', '6', '7', '8', '9', '1', '2'], 
      ['6', '7', '2', '1', '9', '5', '3', '4', '8'], 
...
def generate_sudoku(n: int) -> List[List[str]]:
    ...
    while n + delet < 81:
        ...
        if new_grid is not None:
            if new_grid[random_column][random_row] != ".":
                new_grid[random_column][random_row] = "."
                delet += 1
